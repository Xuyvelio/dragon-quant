...
def detect_pullback_entry(kline, limit_end_pos, observation_window=90):
    ...
    for pos in limit_end_pos:
        ...
        for i in range(break_point + 1, min(break_point + 30, len(post))):
            ...
            if tail or limit:
                signals.append({'entry_date': cur['日期'], 'entry_price': cur['收盘'], 'entry_index': pos + 1 + i, 'ma5_at_entry': cur['MA5'], 'entry_mode': 'limit_up' if limit else 'tail'})
                break
    return signals
